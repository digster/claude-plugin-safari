...
| File | Purpose |
|------|---------|
| `SafariWebExtensionHandler.swift` | Receives `sendNativeMessage` calls from JS. Actions: `runClaude` (uses `NSUserUnixTask` to execute a helper script that invokes the CLI), `cancelClaude` (runs helper script with `--cancel` to kill the running process), `verifyCli` (checks if the helper script is installed), `storeResult`/`getStoredResult`/`clearStoredResult`/`clearAllResults` (disk-based result storage to bypass browser.storage.local quota), `listCachedResults` (lists all per-URL cache files with metadata for the Cache Explorer), `deleteCachedResult` (deletes a single per-URL cache file by URL). `getStoredResult` with a URL returns null on cache miss (no fallback to lastResult). Runs sandboxed. No LRU eviction cap — native disk has no meaningful size constraint; users manage cache via the Cache Explorer. |
...
└── results/                 # Per-URL result cache (SHA-256-hashed filenames)
    ...
    └── ...                     # No cap — managed via Cache Explorer
```
...
| Toolbar badge notification (canvas dot) | Safari's `setBadgeText(' ')` renders a large, fixed-size badge covering ~40% of the icon. To get a smaller dot, we composite directly onto the icon using canvas: `createDotIcon(size, color)` loads the original icon PNG, draws a small circle (~15% of icon size) in the top-right corner, and passes the `ImageData` to `browser.action.setIcon({ imageData, tabId })`. Cached by `"size-color"` key so canvas work happens once per unique combination. Green (#34C759) on complete, red (#FF3B30) on error. Cleared by restoring original icon PNGs via `setIcon({ path })`. Popup sends unconditional `clearBadge` with `tabId` on open. `clearBadgeForTab(tabId)` clears both the icon and all URL→tab map entries. In-memory `urlToTabId` Map (no persistence needed). All badge operations are async and wrapped in try/catch |
...
| Per-URL result cache (`results/` dir) | A singleton `lastResult.json` was overwritten on each new query — revisiting a previous URL found no cache. Now results are also stored in `results/<SHA256-hash>.json` keyed by URL. The popup passes the tab URL when fetching, hitting the per-URL cache. No file cap — native disk has no meaningful size constraint; users manage via the Cache Explorer's Delete/Clear All. `lastResult.json` remains as a "most recent" pointer for the pop-out view |
